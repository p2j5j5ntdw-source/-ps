...
class __SEAXNOOR__:
    ...
    #----------------<-IMPROVED CHECK RESPONSE (الحل النهائي)----------------#
    def _check_response(self, po, ids, pas, method):
        ...
        # مؤشرات تسجيل الدخول الناجح: وجود مفتاح الجلسة أو ملفات تعريف الارتباط
        if 'session_key' in po and 'access_token' in po and 'session_cookies' in po:
            return 'ok', po

        # ========== 2. التحقق من 2FA (المصادقة الثنائية) ==========
        # مؤشرات المصادقة الثنائية: وجود 'error' مع 'error_code' و رسالة تدل على 2FA
        if 'error' in po:
